start_server {tags {"expire"}} {
    ...
    test {TTL / PTTL / EXPIRETIME return -1 if key has no expire} {
        r del x
        r set x hello
        list [r ttl x] [r pttl x] [r expiretime x time] [r expiretime x ptime]
    } {-1 -1 -1 -1}

    test {TTL / PTTL / EXPIRETIME return -2 if key does not exit} {
        r del x
        list [r ttl x] [r pttl x] [r expiretime x time] [r expiretime x ptime]
    } {-2 -2 -2 -2}
    ...
    test {EXPIRETIME returns absolute expiration time in seconds} {
        ...
        assert {[r expiretime x time] eq $abs_expire}
    }

    test {EXPIRETIME returns absolute expiration time in milliseconds} {
        r del x
        set abs_expire [expr [clock milliseconds] + 100000]
        r set x somevalue pxat $abs_expire
        assert {[r expiretime x ptime] eq $abs_expire}
    }
    ...
    start_server {overrides {appendonly {yes} appendfilename {appendonly.aof} appendfsync always}} {
        test {All time-to-live(TTL) in commands are propagated as absolute timestamp in milliseconds in AOF} {
            ...
            # Sleep some to allow AOF to be flushed.
            after 2000
            ...
            set ttl1 [r expiretime foo1 ptime]
            set ttl2 [r expiretime foo2 ptime]
            set ttl3 [r expiretime foo3 ptime]
            set ttl4 [r expiretime foo4 ptime]
            set ttl5 [r expiretime foo5 ptime]
            set ttl6 [r expiretime foo6 ptime]
            set ttl7 [r expiretime foo7 ptime]
            set ttl8 [r expiretime foo8 ptime]
            set ttl9 [r expiretime foo9 ptime]
            set ttl10 [r expiretime foo10 ptime]
            assert {[r expiretime foo11 ptime] eq "-2"} ; # foo11 is gone
            set ttl12 [r expiretime foo12 ptime]
            set ttl13 [r expiretime foo13 ptime]
            set ttl14 [r expiretime foo14 ptime]
            set ttl15 [r expiretime foo15 ptime]
            assert {[r expiretime foo16 ptime] eq "-1"} ; # foo16 has no TTL
            set ttl17 [r expiretime foo17 ptime]
            set ttl18 [r expiretime foo18 ptime]

            # Reload data from AOF
            r debug loadaof
            ...
            assert {[r expiretime foo1 ptime] eq $ttl1}
            assert {[r expiretime foo2 ptime] eq $ttl2}
            assert {[r expiretime foo3 ptime] eq $ttl3}
            assert {[r expiretime foo4 ptime] eq $ttl4}
            assert {[r expiretime foo5 ptime] eq $ttl5}
            assert {[r expiretime foo6 ptime] eq $ttl6}
            assert {[r expiretime foo7 ptime] eq $ttl7}
            assert {[r expiretime foo8 ptime] eq $ttl8}
            assert {[r expiretime foo9 ptime] eq $ttl9}
            assert {[r expiretime foo10 ptime] eq $ttl10}
            assert {[r expiretime foo11 ptime] eq "-2"} ; # foo11 is gone
            assert {[r expiretime foo12 ptime] eq $ttl12}
            assert {[r expiretime foo13 ptime] eq $ttl13}
            assert {[r expiretime foo14 ptime] eq $ttl14}
            assert {[r expiretime foo15 ptime] eq $ttl15}
            assert {[r expiretime foo16 ptime] eq "-1"} ; # foo16 has no TTL
            assert {[r expiretime foo17 ptime] eq $ttl17}
            assert {[r expiretime foo18 ptime] eq $ttl18}
        }
    }
    ...
    start_server {} {
        ...
        test {For all replicated TTL-related commands, absolute expire times are identical on primary and replica} {
            ...
            assert {[$primary wait 1 1000] == 1}

            # Verify absolute TTLs are identical on primary and replica
            # This is because TTLs are always replicated as absolute values
            assert {[$primary expiretime foo1 ptime] eq [$replica expiretime foo1 ptime]}
            assert {[$primary expiretime foo2 ptime] eq [$replica expiretime foo2 ptime]}
            assert {[$primary expiretime foo3 ptime] eq [$replica expiretime foo3 ptime]}
            assert {[$primary expiretime foo4 ptime] eq [$replica expiretime foo4 ptime]}
            assert {[$primary expiretime foo5 ptime] eq [$replica expiretime foo5 ptime]}
            assert {[$primary expiretime foo6 ptime] eq [$replica expiretime foo6 ptime]}
            assert {[$primary expiretime foo7 ptime] eq [$replica expiretime foo7 ptime]}
            assert {[$primary expiretime foo8 ptime] eq [$replica expiretime foo8 ptime]}
            assert {[$primary expiretime foo9 ptime] eq [$replica expiretime foo9 ptime]}
            assert {[$primary expiretime foo10 ptime] eq [$replica expiretime foo10 ptime]}
            assert {[$primary expiretime foo11 ptime] eq [$replica expiretime foo11 ptime]}
            assert {[$primary expiretime foo12 ptime] eq [$replica expiretime foo12 ptime]}
            assert {[$primary expiretime foo13 ptime] eq [$replica expiretime foo13 ptime]}
            assert {[$primary expiretime foo14 ptime] eq [$replica expiretime foo14 ptime]}
            assert {[$primary expiretime foo15 ptime] eq [$replica expiretime foo15 ptime]}
            assert {[$primary expiretime foo16 ptime] eq [$replica expiretime foo16 ptime]}
            assert {[$primary expiretime foo17 ptime] eq [$replica expiretime foo17 ptime]}
        }
    }
